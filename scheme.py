    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil or vals is not nil:
            if formals is nil or vals is nil:
                raise SchemeError
            elif scheme_symbolp(formals):
                new_env.define(formals, vals)
                break
            else:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        if scheme_symbolp(self.formals):
            new_env = self.env.make_child_frame(nil, nil)
            new_env.define(self.formals, args)
            return new_env
        else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.second is nil:
            last = scheme_eval(expressions.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    def list_formals(bindings):
        if bindings is nil:
            return nil
        check_form(bindings.first, 1, 2)
        return Pair(bindings.first.first, list_formals(bindings.second))

    def list_vals(bindings):
        if bindings is nil:
            return nil
        check_form(bindings.first, 1, 2)
        return Pair(scheme_eval(bindings.first.second.first, env), list_vals(bindings.second))

    formals = list_formals(bindings)
    vals = list_vals(bindings)
    check_formals(formals)
